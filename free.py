...
def gif2mp4(gif, mp4):
    ffmpeg_command = [
        'ffmpeg',
        '-i', gif,
        '-vf', 'scale=trunc(iw/2)*2:trunc(ih/2)*2',  # 调整宽度和高度都为偶数
        '-movflags', 'faststart',
...
def mp42gif(input_mp4_filename, output_gif_filename):
    ...
    ffmpeg_palette_command = [
        'ffmpeg',
        '-y',  # 覆盖输出文件
        '-i', input_mp4_filename,  # 输入 MP4 文件
...
def convert_to_jpg(image_path, output_path=None):
    img = cv2.imread(image_path)
    if img is None:
        raise ValueError(f"convert to jpg error: {image_path}")
...
#    return
    ...
    data = callApi("workerFreeTask", {'func':'get', 'term':term})
    print(data)
  #  proc_media('media_filename', 'face_filename', 'out_file_path')
    ...
    try:
        media_file_url = taskData.get('media_url')
        face_file_url = taskData.get('face_url')
    except Exception as e:
    ...
    if media_filename.lower().endswith(('.mp4', '.m4v', '.mkv', '.avi', '.mov', '.webm', '.mpeg', '.mpg', '.wmv', '.flv', '.asf', '.3gp', '.3g2', '.ogg', '.vob', '.rmvb', '.ts', '.m2ts', '.divx', '.xvid', '.h264', '.avc', '.hevc', '.vp9', '.avchd')):
        ...
        now = datetime.now()
        api_res = callApi("workerFreeTask", {'func':'update', 'task_state':3, 'taskId':str(taskData.get('_id', '')), 'out_media_url':upload_video_url, 'thumb_url':upload_image_url, 'file_hash':now.strftime("%Y-%m-%d %H:%M:%S")})
    ...
    if is_image_file(media_filename):
        media_filename = convert_to_jpg(media_filename)
        ...
        api_res = callApi("workerFreeTask", {'func':'update', 'task_state':3, 'taskId':str(taskData.get('_id', '')), 'out_media_url':upload_file_url, 'thumb_url':upload_image_url, 'file_hash':now.strftime("%Y-%m-%d %H:%M:%S")})
        print('Api result:', api_res)
        addLog(1, 3, 'finish', 100)
        return
